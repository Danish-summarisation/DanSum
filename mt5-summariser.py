...
################################ Load data ################################
# 1k
train = Dataset.from_pandas(pd.read_csv("train25k_clean.csv", usecols=['text','summary'])) # training data
test = Dataset.from_pandas(pd.read_csv("test25k_clean.csv", usecols=['text','summary'])) # test data
val = Dataset.from_pandas(pd.read_csv("val25k_clean.csv", usecols=['text','summary'])) # validation data
...
model = AutoModelForSeq2SeqLM.from_pretrained(model_checkpoint)#, min_length=15, max_length=128, num_beams=4, early_stopping=True)
...
save('./' + timestr + '_train.npy', result)  
...
np.save('./' + timestr + '_test.npy', rouge_output)
