...
import pandas as pd
import numpy as np
from transformers import AutoModelForSeq2SeqLM, DataCollatorForSeq2Seq, Seq2SeqTrainingArguments, Seq2SeqTrainer, T5TokenizerFast
import time
import wandb
import accelerate
...
train = Dataset.from_pandas(pd.read_csv("train_clean1.csv", usecols=['text','summary'])) # training data
test = Dataset.from_pandas(pd.read_csv("test_clean1.csv", usecols=['text','summary'])) # test data
val = Dataset.from_pandas(pd.read_csv("val_clean1.csv", usecols=['text','summary'])) # validation data
...
model = AutoModelForSeq2SeqLM.from_pretrained(model_checkpoint, min_length=15, max_length=128, num_beams=4, no_repeat_ngram_size=3, length_penalty=5, early_stopping=True, dropout_rate=0.01)
...
def generate_summary(batch):
    ...
    inputs = tokenizer(batch["text"], padding="max_length", return_tensors="pt", max_length=max_input_length, truncation=True)
